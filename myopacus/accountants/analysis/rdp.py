...
def compute_rdp(
    *, q: float, noise_multiplier: float, steps: int, orders: Union[List[float], float], **kwargs
    ...
    """
    assert q >= 0.0 and q <= 1.0, "The sampling rate `q` must be a positive real number in [0,1]."
...
def _compute_rdp_4fed(inner_rdp: float, client_q: float, rounds: int, alpha: float) -> float:
    if client_q == 1.0: # no outer-level privacy amplification

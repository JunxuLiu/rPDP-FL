...
import torch
from .accountants import create_accountant
from .accountants.utils import get_noise_multiplier
from .data_loader import DPDataLoader, switch_generator
...
from .privacy_personalization_utils import per_sample_rate_estimation
...
class PrivacyEngine:
    ...
    def _prepare_data_loader(
        ...
        *,
        ...
        if poisson_sampling:
            sample_rate = 1 / len(data_loader) if uniform else self.all_sample_rates
            return DPDataLoader.from_data_loader(
                data_loader, generator=self.secure_rng, uniform=uniform, sample_rate = sample_rate, distributed=distributed
            )
    ...
    def make_private(
        ...
        *,
        ...
        data_loader = self._prepare_data_loader(
            data_loader, distributed=distributed, poisson_sampling=poisson_sampling, uniform=True
        )
...
# ===================
    def make_pers_private(
        ...
        *,
        ...
        data_loader = self._prepare_data_loader(
            data_loader, distributed=distributed, uniform=False, poisson_sampling=poisson_sampling, 
        )

        expected_batch_size = int(sum(self.all_sample_rates))
        ...
        optimizer.attach_step_hook(
            self.accountant.get_optimizer_hook_fn(sample_rate=self.all_sample_rates)
        )
    ...
    def make_private_with_personalization(
        ...
        **kwargs,
        ...
        # self.outer_steps = kwargs['outer_rounds']
        # self.outer_rate = kwargs['outer_rate']
        self.all_sample_rates = per_sample_rate_estimation(data_loader.dataset.epsilons,
                                                  delta=target_delta,
                                                  noise_multiplier=noise_multiplier,
                                                  inner_steps=kwargs['inner_steps'],
                                                  outer_rounds=kwargs['outer_rounds'],
                                                  outer_rate=kwargs['outer_rate'],
                                                  plot=False)

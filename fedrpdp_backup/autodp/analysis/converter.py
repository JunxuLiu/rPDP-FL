...
def rdp_to_approxdp(rdp, alpha_max=np.inf, BBGHS_conversion=True):
    ...
    # paper: Hypothesis Testing Interpretations and Rényi Differential Privacy https://arxiv.org/pdf/1905.09982.pdf 
    # Theorem 21. If a mechanism M is (α, ρ)-RDP then it is 
    # (ρ + log((α − 1)/α) − (log δ + log α)/(α − 1), δ)-DP for any 0 < δ < 1.
    ...
    def approxdp(delta):
        ...
        if delta == 0:
            ...
            def fun(x):  # the input the RDP's \alpha 给出 x(i.e. alpha) 的取值范围，求最小函数值
                if x <= 1:
                    return np.inf
                else:
                    if BBGHS_conversion:
            ...
            # minimize_scalar 解一元优化问题，bounds为变量的范围，即求rdp最小时alpha的大小

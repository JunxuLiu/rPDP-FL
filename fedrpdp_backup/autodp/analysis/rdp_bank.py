...
本代码包含常见随机机制的RDP解析式，包括：
[1] 高斯机制 RDP_gaussian
[2] 拉普拉斯机制 RDP_laplace
[3] 随机响应 RDP_randresponse
[4] noisy aggregation: RDP_inde_pate_gaussian(params, alpha) | RDP_depend_pate_gaussian(params, alpha)
    Bounds RDP from above of GNMax given an upper bound on q (Theorem 6).
[5] noisy screen: RDP_independent_noisy_screen | RDP_noisy_screen
    Theorem 7 in Private-kNN

RDP转换
[ ] RDP_pureDP: converts pure DP to Renyi DP. (Lemma 1.4 of Bun et al.'s CDP paper.)

采样机制
[ ] RDP_subsampled_pureDP(params, alpha): The special function for approximating the privacy amplification by subsampling for pure DP mechanism.
        1. This is more or less tight
        2. This applies for both Poisson subsampling (with add-remove DP definition) and Subsampling (with replace DP Definition)
        3. It evaluates in O(1)
[ ] pRDP_asymp_subsampled_gaussian(params, alpha): See Example 19 of Wang, Balle, Kasiviswanathan (2018)
[ ] pRDP_asymp_subsampled_gaussian_best_case(params, alpha): See Example 20 of Wang, Balle, Kasiviswanathan (2018)
...
部分函数可计算给定两个随机分布的renyi divergence，该结果可用来追踪两个给定数据集的per-instance RDP。

ref: https://github.com/imcjp/Opacus-DPCR/blob/65e0709179adb9b4825c957f9248cf3af3196b91/dpcrpy/utils/dpTools/rdp_bank.py
"""
import numpy as np
import math
from . import utils

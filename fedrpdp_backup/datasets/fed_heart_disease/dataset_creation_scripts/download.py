...
sys.path.append("../../..")
from utils import accept_license, create_config, write_value_in_config
import zipfile
...
def main(output_folder, debug=False):
    ...
    accept_license(
        "https://archive.ics.uci.edu/dataset/45/heart+disease", "fed_heart_disease"
    )
    ...
    print("See https://archive.ics.uci.edu/dataset/45/heart+disease.\n")
    ...
    ### modification by Junxu - start
    # for each center, check if downloaded and download if necessary
    zipfile = wget.download(
        "https://archive.ics.uci.edu/static/public/45/heart+disease.zip", out=output_folder
    )
    # unzip the downloaded zipfile
    unzip(zipfile, output_folder)
    ### modification by Junxu - end
    
    for i, center in enumerate(centers):
        file_status_ok = downloaded_status_file.loc[i, "Status"] == "Downloaded"
        if not file_status_ok:
            fname = output_folder + "/processed." + center + ".data"
            print(fname)

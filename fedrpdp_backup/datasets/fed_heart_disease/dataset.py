...
# import sys
# sys.path.append("../..")
from ...utils import check_dataset_from_config
from ...record_budget_generation import EPSILONS_GEN_FUNC
...
class FedHeartDisease(HeartDiseaseRaw):
    ...
    def __init__(
        ...
        if priv_bgt is not None:
            gen_bounded_priv_bgts = lambda priv_bgts: np.array([min(max(x, priv_bgt[1]), priv_bgt[2]) for x in priv_bgts])
            self.epsilons = gen_bounded_priv_bgts(EPSILONS_GEN_FUNC[priv_bgt[0]](len(self.labels)))

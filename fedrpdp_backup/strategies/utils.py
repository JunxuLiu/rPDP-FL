...
from fedrpdp.myopacus import PrivacyEngine
...
class _Model:
    ...
    def __init__(
        ...
        self._apply_fedrpdp = (
            (self.dp_target_epsilon is None)
            and (self.dp_max_grad_norm is not None)
            and (self.dp_target_delta is not None)
            and (self.dp_noise_multiplier is not None)
        )
        ...
        if self._apply_dp:
            ...
            (
                ...
                self._optimizer,
                self._train_dl,
            ) = privacy_engine.make_private_with_personalization(
                module=self.model,
                optimizer=self._optimizer,
                data_loader=self._train_dl,
                epochs=nrounds,
                ...
                outer_rounds=nrounds,
                outer_rate=client_sample_rate
            )
    ...
    def _local_train(self, dataloader_with_memory, num_updates):
        ...
        for _batch in range(num_updates):
            ...
            # )
            if self.log:
                print(f"loss: {_loss:>7f} after {_batch:>5d} steps")
            # if self.log:

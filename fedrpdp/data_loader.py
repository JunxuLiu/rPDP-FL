...
from .utils.uniform_sampler import (
    ...
    NonUniformWithReplacementSampler
)
...
class DPDataLoader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if distributed:
            ...
            assert len(sample_rate) == len(dataset), "the length of sample_rate must be equal to the length of dataset"
    ...
    def from_data_loader(
        cls, data_loader: DataLoader, *, distributed: bool = False, sample_rate: Union[List[float], float] = 1.0, generator=None
    ):
        """
...
def _is_supported_batch_sampler(sampler: Sampler):
    return (
        ...
        or isinstance(sampler, NonUniformWithReplacementSampler)
    )

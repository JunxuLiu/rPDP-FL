...
class _Model:
    ...
    def __init__(
        ...
        if not self.privacy_engine is None: # FedAvg with rPDP
            seed = seed if seed is not None else int(time.time())
            self.privacy_engine.sample_rate = [float(self.privacy_engine.sample_rate_fn(x)) for x in self.dp_target_epsilon] # TODO: make it as an internal func of PrivacyEngine
            make_private_fn = {
                "rdp": self.privacy_engine.make_private,
                "pers_rdp": self.privacy_engine.make_private_with_personalization
            }
            flag = self.privacy_engine.accountant.mechanism()
            (
                self.model,
                self._optimizer,
                self._train_dl,
            ) = make_private_fn[flag](
                module=self.model,
    ...
    def _local_train(self, dataloader_with_memory, num_updates, **kwargs):
        ...
        dataloader_with_memory : DataLoaderWithMemory
            A dataloader that can be called infinitely using its get_samples() method.
        num_updates : int

...
包含两个函数接口：
  1. 计算rdp budget: 相关参数包括采样概率q，噪音参数sigma，更新轮次T，备选orders(alpha)。其中，sigma is the normalized noise level: std divided by global L2 sensitivity
  2. 计算privacy spent (相对于privacy budget而言): 给定一系列备选orders(alpha)的 rdp budgets，它们对应的 eps (给定 delta) 就是privacy spent。

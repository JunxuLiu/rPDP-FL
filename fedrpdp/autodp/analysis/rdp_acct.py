...
* The RDP accountant API is deprecated. 弃用
...
from ..calibrator.privacy_calibrator import subsample_epsdelta
import scipy
import math
...
class anaRDPacct:
    ...
    CGF: Cumulant generating function -- the natural logarithm of its moment generating function.
         Remember that the moment generating function (mgf) of a random variable X is defined as M_X[alpha] = E[exp(tX)]
         then the function K_X[alpha] = ln(M_X[t]) is the CGF of X.
    
    the renyi divergence = 1/(alpha-1) * log E_q[(p(x)/q(x))^alpha]
    if we look at `p(x)/q(x)` as a random variable, that is, the renyi divergence of two distributions p(x) & q(x) is the CGF of `p(x)/q(x)` scaled with `1/(alpha-1)`
    """

...
from ..mechanisms.mechanism_base import Mechanism
from ..mechanisms import mechanism_zoo
from ..analysis import rdp_acct
...
class Composition(Transformer):
    ...
    输入a list of Mechanisms，输出the composed mechanism
    """
    def __init__(self):
    ...
    def compose(self, mechanism_list, coeff_list, RDP_compose_only=True):
        ...
        # newmech 就是最终要返回的组合机制，这里先初始化为 Mechanism 类型
        newmech = Mechanism()
...
class AmplificationBySampling(Transformer):
    ...
    def amplify(self, mechanism, prob, improved_bound_flag=False):
        ...
        # acct.evalRDP = lambda x: sum([c * item(x) for (c, item) in zip(self.coeffs, self.RDPs)])
        # 该函数输入alpha，输出为 c*RDPs(alpha)

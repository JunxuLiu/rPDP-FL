...
def evaluate_model_on_tests(
    model, test_dataloaders, metric, use_gpu=True, device="cuda:0", return_pred=False
):
    ...
    with torch.no_grad():
        for i in range(len(test_dataloaders)):
            ...
            for (X, y) in test_dataloader_iterator:
                ...
                y_pred_final.append(y_pred.detach().cpu().numpy())
                y_true_final.append(y.detach().cpu().numpy())

            y_true_final = np.concatenate(y_true_final)
            y_pred_final = np.concatenate(y_pred_final)
            
            correct = metric(y_true_final, y_pred_final)
            results_dict[f"client_test_{i}"] = correct

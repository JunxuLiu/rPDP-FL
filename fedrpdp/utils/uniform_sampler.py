...
class NonUniformWithReplacementSampler(Sampler[List[int]]):
    ...
    def __init__(
        self, *, num_samples: int, sample_rate: Union[List[float],float], generator=None
    ):
        r"""
        Args:
            num_samples: number of samples to draw.
        ...
        #         "value, but got steps={}".format(steps)
        #     )
    ...
    def __iter__(self):
        ...
        while num_batches > 0:
            ...
            indices = mask.nonzero(as_tuple=False).reshape(-1).tolist()
            # print(f'Num of selected points: {len(indices)} / {self.num_samples}')
            yield indices
